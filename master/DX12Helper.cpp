...
Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> DX12Helper::GetImGuiDescriptorHeap()
{
	return cpuSideImGuiHeap;
}


void DX12Helper::LoadImGui()
{
	...
	device->CreateDescriptorHeap(&dhDesc, IID_PPV_ARGS(cpuSideImGuiHeap.GetAddressOf()));
	D3D12_CPU_DESCRIPTOR_HANDLE cpuHandle = cpuSideImGuiHeap->GetCPUDescriptorHandleForHeapStart();
	imGuiCPUHandle = cpuHandle;
}
...
D3D12_GPU_DESCRIPTOR_HANDLE DX12Helper::CreateImGuiGPUHandle(D3D12_CPU_DESCRIPTOR_HANDLE firstDescriptorToCopy)
{
	D3D12_GPU_DESCRIPTOR_HANDLE gpuHandle = cbvSrvDescriptorHeap->GetGPUDescriptorHandleForHeapStart();
...
void DX12Helper::CreateCBVSRVDescriptorHeap()
{
	...
	dhDesc.NumDescriptors = maxConstantBuffers + maxTextureDescriptors; // How many descriptors will we need? (Make sure both sents of descriptors are accounted for)
	...
	// Assume the first SRV will be after all possible CBVs (Don't forget this otherwise everything breaks)
	srvDescriptorOffset = maxConstantBuffers;
}

//Create a single CBV descriptor heap which holds all 
//CBVs for the entire program and allows re-use of memory.
void DX12Helper::CreateImGuiDescriptorHeap()
{
	// Ask the device for the increment size for CBV descriptor heaps
	// This can vary by GPU so we need to query for it
	cbvSrvDescriptorHeapIncrementSize = (SIZE_T)device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
